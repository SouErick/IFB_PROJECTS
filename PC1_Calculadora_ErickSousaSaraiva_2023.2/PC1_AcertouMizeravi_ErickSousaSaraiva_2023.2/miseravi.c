...
void remover_zeros(char numero[]) // Função remover zeros à esquerda.
{
    ...
    if (cont_zero > 0) // Se o '0' for encontrado.
    {
        ...
        numero[posicao] = '\0'; // Termina a string.
    }
}
void subtracao(char primeiro_numero[], char segunda_numero[], char resultado[]) // Função subtração.
{
...
void somar(char primeiro_numero[], char segundo_numero[], char resultado[]) // Função Soma.
{
    ...
    for (int i = 0; i < tamanho_maior; i++)                          // Percorrer a string do maior numero garantindo que a soma fique organizada igual no papel "um abaixo do outro".
    {
        ...
        {
            digito_primeiro = 0; // Quando o contador for maior que o tamanho do primeiro número, ele pode continuar percorrendo o tamanho do maior numero entre as comparações, assim o valor alocado nos digitos será zero.
        }
